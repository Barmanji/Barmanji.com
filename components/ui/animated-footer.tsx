...
export function AnimatedFooter({
  ...
  useEffect(() => {
    ...
    loadHand(leftImage, leftWrap.querySelector("canvas")!, 1);
    loadHand(rightImage, rightWrap.querySelector("canvas")!, -1);

    const renderHand = (hand: Hand, now: number) => {
      const { ctx, cellList, cellSize: cs, baselineOffset, columns: cols } = hand;
      const { charColor: cc, hoverColor: hc, hoverCharColor: hcc } = liveRef.current;
      ctx.clearRect(0, 0, cols * cs, hand.rows * cs);
