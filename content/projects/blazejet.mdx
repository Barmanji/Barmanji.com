...
This document provides a comprehensive overview of the VercelClone project, detailing its architecture, the technologies employed across its services, and the exposed API endpoints. Made with the help of [Piyush Garg](https://github.com/piyushgarg-dev) - But could never run it (It runs manually, build the docker images to ECS in AWS, I could never figure out how to do that programmatically)
